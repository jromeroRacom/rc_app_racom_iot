...
export class MqttUsersTableDataSource implements DataSource<MqttUsersTableModel> {

  private errorEvent$ = new Subject<HttpErrorResponse>();      // Manejador de errores
  dataChange = new BehaviorSubject<MqttUsersTableModel[]>([]);  // Cambios en datos
  filterChange = new BehaviorSubject('');                      // Cambios en filtro
  filteredData: MqttUsersTableModel[] = [];                     // Datos filtrados
  renderedData: MqttUsersTableModel[] = [];                     // Datos mostrados
  public flagLoading = true;                                   // Bamdera para detectar loading
  private destroy$ = new Subject();                            // Observable para acciones de destroy

  /** Constructor */
  constructor(
    public tableService: MqttUsersTableService,      // Servicio de tabla
  ...
  get data(): MqttUsersTableModel[] { return this.dataChange.value; }
  /* --------------------------------------- */
  ...
  getDataFromService(): void {
    ...
    this.tableService.getMqttUsers()                 // Petición para obtener datos de tabla
      .pipe(
      ...
      .subscribe((data) => {                          // Subscripción
        this.dataChange.next(data.users?.rows);   // Pone nuevos datos
  ...
  connect( collectionViewer: CollectionViewer): Observable<MqttUsersTableModel[]> {
    ...
    return merge(...displayDataChanges).pipe(  // Retorna conjunto de eventos
      map(() => {
        ...
        this.filteredData = this.data                   // Obtiene datos
          ...
          .filter((deviceRow: MqttUsersTableModel) => { // Filtra cada columna con filtro
  ...
  sortData(data: MqttUsersTableModel[]): MqttUsersTableModel[] {
