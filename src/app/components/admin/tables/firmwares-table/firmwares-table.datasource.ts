...
export class FirmwaresTableDataSource implements DataSource<FirmwareTableModel> {

  private errorEvent$ = new Subject<HttpErrorResponse>();      // Manejador de errores
  dataChange = new BehaviorSubject<FirmwareTableModel[]>([]);  // Cambios en datos
  filterChange = new BehaviorSubject('');                      // Cambios en filtro
  filteredData: FirmwareTableModel[] = [];                     // Datos filtrados
  renderedData: FirmwareTableModel[] = [];                     // Datos mostrados
  public flagLoading = true;                                   // Bamdera para detectar loading
  private destroy$ = new Subject();                            // Observable para acciones de destroy

  /** Constructor */
  constructor(
    public tableService: FirmwaresTableService,      // Servicio de tabla
  ...
  get data(): FirmwareTableModel[] { return this.dataChange.value; }
  /* --------------------------------------- */
  ...
  getDataFromService(): void {
    ...
    this.tableService.getFirmwares()                 // Petición para obtener datos de tabla
      .pipe(
      ...
      .subscribe((data) => {                          // Subscripción
        this.dataChange.next(data.firmwares?.rows);   // Pone nuevos datos
  ...
  connect( collectionViewer: CollectionViewer): Observable<FirmwareTableModel[]> {
    ...
    return merge(...displayDataChanges).pipe(  // Retorna conjunto de eventos
      map(() => {
        ...
        this.filteredData = this.data                   // Obtiene datos
          ...
          .filter((deviceRow: FirmwareTableModel) => { // Filtra cada columna con filtro
  ...
  sortData(data: FirmwareTableModel[]): FirmwareTableModel[] {
