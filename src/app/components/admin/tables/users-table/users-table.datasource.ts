...
export class UsersTableDataSource implements DataSource<UsersTableModel> {

  private errorEvent$ = new Subject<HttpErrorResponse>();      // Manejador de errores
  dataChange = new BehaviorSubject<UsersTableModel[]>([]);  // Cambios en datos
  filterChange = new BehaviorSubject('');                      // Cambios en filtro
  filteredData: UsersTableModel[] = [];                     // Datos filtrados
  renderedData: UsersTableModel[] = [];                     // Datos mostrados
  public flagLoading = true;                                   // Bamdera para detectar loading
  private destroy$ = new Subject();                            // Observable para acciones de destroy

  /** Constructor */
  constructor(
    public tableService: UsersTableService,      // Servicio de tabla
  ...
  get data(): UsersTableModel[] { return this.dataChange.value; }
  /* --------------------------------------- */
  ...
  getDataFromService(): void {
    ...
    this.tableService.getUsers()                 // Petición para obtener datos de tabla
      .pipe(
  ...
  connect( collectionViewer: CollectionViewer): Observable<UsersTableModel[]> {
    ...
    return merge(...displayDataChanges).pipe(  // Retorna conjunto de eventos
      map(() => {
        ...
        this.filteredData = this.data                   // Obtiene datos
          ...
          .filter((deviceRow: UsersTableModel) => { // Filtra cada columna con filtro
  ...
  sortData(data: UsersTableModel[]): UsersTableModel[] {
